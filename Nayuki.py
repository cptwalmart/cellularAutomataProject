...
class QuadraticSurdField(Field):
    ...
    def add(self, x, y):
        ...
        return QuadraticSurd(
            x.a * y.c + y.a * x.c, x.b * y.c + y.b * x.c, x.c * y.c, self.d
        )

    def reciprocal(self, x):
        self._check(x)
        return QuadraticSurd(
            -x.a * x.c, x.b * x.c, x.b * x.b * self.d - x.a * x.a, self.d
        )
    ...
    def multiply(self, x, y):
        ...
        self._check(y)
        return QuadraticSurd(
            x.a * y.a + x.b * y.b * self.d, x.a * y.b + y.a * x.b, x.c * y.c, self.d
        )

    def _check(self, x):
        if x.d != self.d:
            raise ValueError(
                ("The value under the square root must match that of the field")
            )
...
class Matrix:
    ...
    def add_rows(self, srcrow, destrow, factor):
        ...
        if not (0 <= srcrow < len(self.values) and 0 <= destrow < len(self.values)):
            raise IndexError("Row index out of bounds")
        self.values[destrow] = [
            self.f.add(destval, self.f.multiply(srcval, factor))
        ...
        for i in range(rows):
            for j in range(cols):
                ...
                for k in range(cells):
                    sum = self.f.add(
                        self.f.multiply(self.get(i, k), other.get(k, j)), sum
                    )
                result.set(i, j, sum)
        return result
    ...
    def reduced_row_echelon_form(self):
        ...
        for j in range(cols):  # For each column
            if numpivots >= rows:
                break
            pivotrow = numpivots
            while pivotrow < rows and self.f.equals(
                self.get(pivotrow, j), self.f.zero()
            ):
                pivotrow += 1
            if pivotrow == rows:
        ...
        for i in reversed(range(numpivots)):
            # Find pivot
            pivotcol = 0
            while pivotcol < cols and self.f.equals(
                self.get(i, pivotcol), self.f.zero()
            ):
                pivotcol += 1
            if pivotcol == cols:
    ...
    def find_free_variables(self):
        ...
        for i in range(rows):
            for j in range(pivotcol, cols):
                if  j <= pivotcol and self.f.equals(self.get(i, j), self.f.zero()):
                    pivotcol+= 1
                if  j > pivotcol and not self.f.equals(self.get(i, j), self.f.zero()):
                    if(not(j in freevariables)):
                        freevariables.append(j)
    ...
    def find_pivots(self):
        ...
        for i in range(rows):  # For each column
            for j in range(pivotcol, cols):
                if not self.f.equals(self.get(i, j), self.f.zero()):
                    pivots.append(pivotcol)
                    pivotcol += 1
                    break
                else:
                    pivotcol += 1
    ...
    def invert(self):
        ...
        for i in range(rows):
            for j in range(cols):
                if not self.f.equals(
                    temp.get(i, j), (self.f.one() if i == j else self.f.zero())
                ):
...
# B.set(1,0,2), B.set(1,1,4), B.set(1,2,0)
# B.set(2,0,1), B.set(2,1,1), B.set(2,2,3)
